...
{% hint style="danger" %}
Notice the implementation of the scaler below is incorrect since each data transformed using its own parameters. This leads to train-test contamination, since we will get an idea about the mean and standart deviation of test data.
{% endhint %}
...
{% hint style="success" %}
Correct implementation of scaling is that we first transform the training data and then using those parameters we fit each of the data accordingly. This way we will only use mean and standard deviation obtained from training data and scale test data using those parameters; hence, we will avoid train-test contamination.
{% endhint %}
...
std_test  = scaler2.fit(test_data)
```
